...
def test_cache(ticker, date, price, price_cache):
    with HTTMock(mock.mock_prices):
        get_daily_price_history(ticker=ticker, start_date=test_settings.START, end_date=test_settings.END)
    cache_results = price_cache.filter_price_cache(ticker=ticker,start_date=date, end_date=date)
    assert(len(cache_results)==1 and cache_results[date][keys.keys['PRICES']['CLOSE']] == price)
...
def test_past_interest(maturity, date, yield_rate, interest_cache):
    ...
    cache_results = interest_cache.filter_interest_cache(maturity=maturity, start_date=date, end_date=date)
