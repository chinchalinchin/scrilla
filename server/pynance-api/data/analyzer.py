...
def market_queryset_gap_analysis(symbol, start_date=None, end_date=None):
    ...
    if asset_type == app_settings.ASSET_EQUITY:
        ...
        queryset = EquityMarket.objects.filter(ticker=ticker[0], date__gte=start_date, date__lte=end_date)
        ...
        queryset = CryptoMarket.objects.filer(ticker=ticker[0], date__gte=start_date, date__lte=end_date)
