...
def percentile_test():
    ...
    twenty_fifth = estimators.sample_percentile(
        data=x_y_data[1], percentile=0.25)
    fiftieth = estimators.sample_percentile(data=x_y_data[1], percentile=0.5)
    seventy_fifth = estimators.sample_percentile(
        data=x_y_data[1], percentile=0.75)
    ninetieth = estimators.sample_percentile(data=x_y_data[1], percentile=0.90)

    outputter.print_separator_line()
    outputter.scalar_result(calculation='25th percentile',
                            result=twenty_fifth, currency=False)
    outputter.scalar_result(calculation='50th percentile',
                            result=fiftieth, currency=False)
    outputter.scalar_result(calculation='75th percentile',
                            result=seventy_fifth, currency=False)
    outputter.scalar_result(calculation='90th percentile',
                            result=ninetieth, currency=False)
    outputter.print_separator_line()
...
def simple_regression_test():
    ...
    # TEST RESULTS
    outputter.scalar_result(calculation='correct correlation',
                            result=x_y_correl, currency=False)
    outputter.scalar_result(
        calculation='sample_correlation(x, y)',  result=correl, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation='correct regression slope', result=x_y_beta, currency=False)
    outputter.scalar_result(
        calculation='simple_regression_beta(x,y)',  result=beta, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation='correct regression intercept',  result=x_y_alpha, currency=False)
    outputter.scalar_result(
        calculation='simple_regression_alpha(x,y)',  result=alpha, currency=False)
    outputter.print_separator_line()
...
def rolling_recursion_test():
    ...
    # ACTUAL COVARIANCES
    covar_1 = estimators.sample_covariance(
        x=rolling_x_y_1[0], y=rolling_x_y_1[1])
    covar_2 = estimators.sample_covariance(
        x=rolling_x_y_2[0], y=rolling_x_y_2[1])
    covar_3 = estimators.sample_covariance(
        x=rolling_x_y_3[0], y=rolling_x_y_3[1])
    covar_4 = estimators.sample_covariance(
        x=rolling_x_y_4[0], y=rolling_x_y_4[1])
    ...
    # TEST RESULTS
    outputter.scalar_result(calculation="Actual Mean 2",
                            result=x_mean_2, currency=False)
    outputter.scalar_result(calculation="Recursive Mean 2",
                            result=recursive_x_mean_2, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(calculation="Actual Y Mean 2",
                            result=y_mean_2, currency=False)
    outputter.scalar_result(calculation="Recursive Y Mean 2",
                            result=recursive_y_mean_2, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation="Actual Variance 2", result=var_2, currency=False)
    outputter.scalar_result(
        calculation="Recursive Variance 2", result=recursive_var_2, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation="Actual Covariance 2", result=covar_2, currency=False)
    outputter.scalar_result(
        calculation="Recursive Covariance 2", result=recursive_covar_2, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(calculation="Actual Mean 3",
                            result=x_mean_3, currency=False)
    outputter.scalar_result(calculation="Recursive Mean 3",
                            result=recursive_x_mean_3, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(calculation="Actual Y Mean 3",
                            result=y_mean_3, currency=False)
    outputter.scalar_result(calculation="Recursive Y Mean 3",
                            result=recursive_y_mean_3, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation="Actual Variance 3", result=var_3, currency=False)
    outputter.scalar_result(
        calculation="Recursive Variance 3", result=recursive_var_3, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation="Actual Covariance 3", result=covar_3, currency=False)
    outputter.scalar_result(
        calculation="Recursive Covariance 3", result=recursive_covar_3, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(calculation="Actual Mean 4",
                            result=x_mean_4, currency=False)
    outputter.scalar_result(calculation="Recursive Mean 4",
                            result=recursive_x_mean_4, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(calculation="Actual Y Mean 4",
                            result=y_mean_4, currency=False)
    outputter.scalar_result(calculation="Recursive Y Mean 4",
                            result=recursive_y_mean_4, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation="Actual Variance 4", result=var_4, currency=False)
    outputter.scalar_result(
        calculation="Recursive Variance 4", result=recursive_var_4, currency=False)
    outputter.print_separator_line()
    outputter.scalar_result(
        calculation="Actual Covariance 4", result=covar_4, currency=False)
    outputter.scalar_result(
        calculation="Recursive Covariance 4", result=recursive_covar_4, currency=False)
    outputter.print_separator_line()


def rolling_recursion_tests_with_financial_data():
    trading_period = static.functions.get_trading_period(
    ...
    for ticker in test_tickers:
        for date in test_dates:
            ...
            prices = services.get_daily_price_history(ticker=ticker,
                                                      start_date=dater.parse_date_string(
                                                          previous_start_date),
                                                      end_date=dater.parse_date_string(
                                                          date),
                                                      asset_type=static.keys['ASSETS']['EQUITY'])
            ...
            new_return = numpy.log(
                float(end_date_price)/float(previous_end_date_price))/trading_period
            lost_return = numpy.log(
                float(start_date_price)/float(previous_start_date_price))/trading_period
            new_mod_return = new_return*numpy.sqrt(trading_period)
            lost_mod_return = lost_return*numpy.sqrt(trading_period)

            old_profile = statistics.calculate_moment_risk_return(
                ticker=ticker, sample_prices=previous_prices)
            old_var = old_profile['annual_volatility']**2
            old_mod_return = old_profile['annual_return'] * \
                numpy.sqrt(trading_period)

            new_actual_profile = statistics.calculate_moment_risk_return(
                ticker=ticker, sample_prices=new_prices)
            ...
            new_recursive_profile['annual_volatility'] = numpy.sqrt(
                new_recursive_profile['annual_volatility'])
            # ito's lemma
            # new_recursive_profile['annual_return'] = new_recursive_profile['annual_return'] - 0.5 * (new_recursive_profile['annual_volatility'] ** 2)

            outputter.scalar_result(
                calculation=f'{ticker}_return({date})_actual', result=new_actual_profile['annual_return'], currency=False)
            outputter.scalar_result(calculation=f'{ticker}_return({date})_recursive',
                                    result=new_recursive_profile['annual_return'], currency=False)
            outputter.scalar_result(calculation=f'{ticker}_vol({date})_actual',
                                    result=new_actual_profile['annual_volatility'], currency=False)
            outputter.scalar_result(calculation=f'{ticker}_vol({date})_recursive',
                                    result=new_recursive_profile['annual_volatility'], currency=False)
